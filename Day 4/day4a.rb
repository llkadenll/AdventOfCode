...
def mark_number(number, boards, checked_boards)
  boards.each_with_index do |board, board_index|
    board.each_with_index do |row, row_index|
      i = row.index(number)
      unless i.nil?
        checked_boards[board_index][row_index][i] = true
        return board_index if check_bingo(checked_boards[board_index][row_index])
        return board_index if check_bingo(checked_boards[board_index].map { |row| row[row_index] })
      end
    end
  end
...
def find_bingo(numbers, boards, checked_boards)
  numbers.each do |number|
    winner = mark_number(number, boards, checked_boards)
    unless winner.nil?
